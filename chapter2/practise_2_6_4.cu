...
__global__ void sumMatrixOnDevicev2(float *MatA, float *MatB, float *MatC, const int nx, const int ny){
    unsigned int ix = threadIdx.x + blockIdx.x*blockDim.x; 
    unsigned int iy = blockIdx.y;
    ...
    if(ix < nx && iy < ny){
        for(int i=0; i<2; i++){
            MatC[idx+i] = MatA[idx+i] + MatB[idx+i];
        }
    }
}
...
int main(){
    ...
    dim3 grid((nx+block.x-1)/block.x/2, ny);

    iStart = cpuSecond();
    //invoke the kernel
    sumMatrixOnDevicev2<<<grid, block>>>(d_MatA, d_MatB, d_MatC, nx, ny);
