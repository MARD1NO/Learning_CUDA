...
__global__ void transposeRow(float *out, float *in, const int nx, const int ny){
    unsigned int ix = blockDim.x * blockIdx.x + threadIdx.x; 
    unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y; 
    unsigned int row = gridDim.x * blockDim.x * iy + ix; 
    
    if(row < ny){
        int row_start = row * nx; 
        int row_end = (row+1)*nx; 
        int column_index = row;
        for(int i = row_start; i < row_end; i++){
            out[column_index] = in[i];
            column_index += nx; 
        }
    }
}
...
__global__ void transposeUnroll8Row(float *out, float *in, const int nx, const int ny){
    ...
    if (ix+7*blockDim.x < nx && iy < ny){
        ...
        out[to + 2*blockDim.x*ny] = in[ti + 2*blockDim.x]; 
        out[to + 3*blockDim.x*ny] = in[ti + 3*blockDim.x]; 
        out[to + 4*blockDim.x*ny] = in[ti + 4*blockDim.x]; 
        out[to + 5*blockDim.x*ny] = in[ti + 5*blockDim.x]; 
        out[to + 6*blockDim.x*ny] = in[ti + 6*blockDim.x]; 
        out[to + 7*blockDim.x*ny] = in[ti + 7*blockDim.x]; 
    }
}
