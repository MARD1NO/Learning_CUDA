...
int main(int argc, char **argv)
{
    ...
    // Compute the time of Memory Copy from host to device
    CHECK(cudaEventRecord(start, 0)); // set to the default stream 
    ...
    // Compute the time of Memory Copy from device to host
    CHECK(cudaEventRecord(start, 0)); // set to the default stream 
